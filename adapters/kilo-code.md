...
detection:
  primary: ".kilocodemodes contains slug: legion, or ~/.kilocode/globalStorage/kilocode.kilo-code/settings/custom_modes.yaml contains slug: legion"
  secondary: ".kilo/commands/legion-start.md, ~/.config/kilo/commands/legion-start.md, .kilo/skills/board-of-directors/SKILL.md, ~/.kilo/skills/board-of-directors/SKILL.md, .kilocode/skills/legion/SKILL.md, or ~/.kilocode/skills/legion/SKILL.md exists"
...
Kilo Code plugin support is distinct from the Kilo CLI adapter. Legion installs a single Kilo Code custom mode named `Legion`, native workflow files such as `/legion-start` and `/legion-board`, and the Legion Agent Skills collection. It does not create one mode per Legion command or personality, and it does not claim native `/legion:*` slash-command discovery inside the IDE plugin.

## Tool Mappings

| Generic Concept | Implementation |
|-----------------|----------------|
| `spawn_agent_personality` | Select the `Legion` mode, then route through the installed `/legion-*` workflow or matching command markdown under `.legion/commands/legion/`. |
| `spawn_agent_autonomous` | Execute the matching native Kilo workflow file from `.kilo/commands/` or `~/.config/kilo/commands/`. |
...
| `global_config_dir` | `~/.config/kilo/commands/`, `~/.kilo/skills/`, `~/.kilocode/skills/legion/`, plus `~/.kilocode/globalStorage/kilocode.kilo-code/settings/custom_modes.yaml`. |
| `plugin_discovery_glob` | `.kilo/commands/legion-start.md`, `.kilo/skills/<name>/SKILL.md`, `.kilocodemodes`, `~/.config/kilo/commands/legion-start.md`, `~/.kilo/skills/<name>/SKILL.md`, or `~/.kilocode/globalStorage/kilocode.kilo-code/settings/custom_modes.yaml`. |
...
1. When a user asks for Legion or a `/legion:*` workflow, use the installed `/legion-*` workflow, the `Legion` custom mode, or the `legion` bridge skill.
...
|-------|--------|------------|
| `plugin-mode-selection` | Users may select the `Legion` mode directly or run native `/legion-*` workflow files. | Keep the generated mode description and workflow metadata clear. |
