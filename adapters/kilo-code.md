...
Kilo Code plugin support is distinct from the Kilo CLI adapter. Legion installs a Kilo Code custom mode named `Legion` and a global or project `legion` skill. It does not install Kilo CLI command wrappers, and it does not claim native `/legion:*` slash-command discovery inside the IDE plugin.

## Tool Mappings

| Generic Concept | Implementation |
|-----------------|----------------|
| `spawn_agent_personality` | Select the `Legion` mode, then route through the installed command markdown under `.legion/commands/legion/`. |
| `spawn_agent_autonomous` | Execute the matching Legion workflow directly from the installed command file. |
...
1. When a user asks for Legion or a `/legion:*` workflow, use the installed `legion` skill or the `Legion` custom mode.
...
|-------|--------|------------|
| `plugin-mode-selection` | Users select the `Legion` mode instead of running native Legion slash-command files. | Keep the generated mode description and skill metadata clear. |
