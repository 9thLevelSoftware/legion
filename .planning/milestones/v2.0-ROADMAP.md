# The Agency Workflows — v2.0 Roadmap (Archived)

**Milestone:** v2.0 — Proper Claude Code Plugin
**Status:** Complete
**Timeline:** 2026-03-01 → 2026-03-02
**Phases:** 9 (15-23)
**Plans:** 9
**Commits:** 23
**Requirements:** 26/26 satisfied
**Lines changed:** +19,470 / -621

## Key Accomplishments

1. **Plugin structure** — `.claude-plugin/plugin.json` manifest, `settings.json`, `commands/` / `skills/` / `agents/` at plugin root for Claude Code auto-discovery
2. **Full migration** — 51 agents, 17 skills, 10 commands moved from `.claude/` to plugin-relative paths with zero stale references
3. **Distribution artifacts** — marketplace.json, README with install guide, CHANGELOG, CONTRIBUTING for `claude plugin add github:9thLevelSoftware/agency-agents`
4. **Strategic Advisors** — `/agency:advise` command for read-only expert consultation with three-layer safety (allowed-tools + Explore subagent + prompt)
5. **Dynamic Review Panels** — 2-4 reviewer composition with domain-weighted rubrics and non-overlapping criteria in `/agency:review`
6. **Plan Critique** — Pre-mortem analysis and assumption hunting as optional Step 8.5 in `/agency:plan`

## Phases

- [x] **Phase 15: Plugin Scaffold** - Create the `.claude-plugin/plugin.json` manifest and `settings.json`; establish the root directory structure every subsequent phase builds into
- [x] **Phase 16: Agent Migration** - Move all 51 agent files to plugin `agents/` directory with updated frontmatter (name, description, division fields)
...
- [x] **Phase 18: Command Migration and Path Updates** - Move 9 commands to plugin `commands/` directory and update all cross-references throughout the codebase
- [x] **Phase 19: Registry Integration** - Update agent-registry skill to reference new plugin-relative agent paths so lookup and recommendation work correctly
...
- [x] **Phase 21: Strategic Advisors** - Add `/agency:advise` command for read-only consultation using Agency's 51 agent personalities as domain experts
- [x] **Phase 22: Dynamic Review Panels** - Compose context-aware multi-perspective review teams with domain-weighted scoring rubrics
- [x] **Phase 23: Plan Critique** - Pre-mortem analysis and assumption hunting for stress-testing plans before execution
...
| 23. Plan Critique | 1/1 | Complete | 2026-03-02 |
