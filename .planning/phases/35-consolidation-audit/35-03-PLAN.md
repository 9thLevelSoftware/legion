...
must_haves:
  truths:
    - >-
      workflow-common/SKILL.md has a dedicated Agent Team Conventions section
      documenting the full Teams lifecycle (TeamCreate, spawn with team_name,
      SendMessage, shutdown, TeamDelete)
    - >-
      workflow-common/SKILL.md documents the
      CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS flag requirement
    - >-
      workflow-common/SKILL.md documents that teammates cannot spawn nested
      teams
    - >-
      memory-manager/SKILL.md has a section documenting the relationship between
      Legion memory and Claude Code memory
    - >-
      memory-manager/SKILL.md defines the boundary: Legion memory is
      project-local agent orchestration data, Claude Code memory is
      platform-level auto-managed context
    - >-
      memory-manager/SKILL.md states Legion reads from Claude Code memory when
      available but does NOT write to it
    - >-
      workflow-common/SKILL.md Memory Conventions section mentions Claude Code
      memory integration
  artifacts:
  ...
  key_links:
    - from: skills/workflow-common/SKILL.md
      ...
      via: >-
        Agent Team Conventions section references wave-executor as the primary
        implementer
      pattern: wave-executor
    - from: skills/workflow-common/SKILL.md
      to: skills/review-loop/SKILL.md
      via: >-
        Agent Team Conventions section references review-loop as a Teams
        implementer
      pattern: review-loop
    - from: skills/memory-manager/SKILL.md
      to: skills/workflow-common/SKILL.md
      via: >-
        Memory alignment section references Memory Conventions in
        workflow-common
