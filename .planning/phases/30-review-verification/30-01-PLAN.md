...
must_haves:
  truths:
    - >-
      review-loop Section 3 review prompt includes confidence classification
      instructions (HIGH/MEDIUM/LOW with percentage ranges)
    - >-
      review-loop Section 4 triage filters findings by confidence level — only
      HIGH (80%+) findings enter the must-fix list by default
    - >-
      review-panel Section 2 rubric injection includes confidence classification
      requirement
    - >-
      review-loop Section 6 tracks finding deltas between cycles and aborts when
      no progress is detected
    - >-
      Stale loop abort produces a structured report of remaining issues with
      recommendations
  artifacts:
